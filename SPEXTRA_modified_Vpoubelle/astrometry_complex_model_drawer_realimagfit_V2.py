...
from common_tools import wrap, reorder_baselines, get_all_baseline_names, mas2rad, OB_association_number
...
#Coherent flux model
C_model = 2.258871822415009e-05
R_model = 1.7926850737150732e-08
offset_RA = mas2rad(106.642)
offset_Dec = mas2rad(-145.540)


# Hypothesis on the planet-to-star contrast (not fitted with the astrometry)
Cps_file = '/data/home/jscigliuto/Pipeline/Templates/HD72946/contrast_template_bt-settl_hd72946_ph1ld.fits'
#Cps_file = 'flat 7e-4'
#Cps = 7e-4
...
def mas2rad(x):
    return x * (np.pi/180) * (1/3600000)

def model(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_iB):
    # Compute the fitted polynomial stellar residual contrast
    # n_coeffs = stellar_residuals_poly_coeffs.size // 2
    n_coeffs = stellar_residuals_poly_coeffs.size 
    Css_real = Polynomial(stellar_residuals_poly_coeffs[:n_coeffs])(wl)
    # Css_real = stellar_residuals_poly_coeffs[0]*wl + stellar_residuals_poly_coeffs[1]
    # print('verify', stellar_residuals_poly_coeffs[0], stellar_residuals_poly_coeffs[1], wl)
    ...
    # a= 1e-2
    # b= -0.04
    # model = transmission_ratio * Cps * ( np.cos(2 * np.pi * freqProj_iB) - 1j * np.sin(2 * np.pi * freqProj_iB)) + a*wl +b 
    # Cps = 0.001
    transmission_ratio = 1
    model = transmission_ratio * Cps * (np.cos(2 * np.pi * freqProj_iB) + 1j * np.sin(2 * np.pi * freqProj_iB)) + Css_real
    # model = transmission_ratio * Cps + Css_real * np.cos(2 * np.pi * freqProj_iB) + 1j * Css_imag * np.sin(2 * np.pi * freqProj_iB)
    amp_model = np.abs(model)
    phi_model = np.angle(model)
    return amp_model, phi_model

def model2(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_iB):
    # Compute the fitted polynomial stellar residual contrast
    n_coeffs = stellar_residuals_poly_coeffs.size 
    Css_real = Polynomial(stellar_residuals_poly_coeffs[:n_coeffs])(wl)
    # Css_real = stellar_residuals_poly_coeffs[0]*wl + stellar_residuals_poly_coeffs[1]
    ...
    amp_model = np.abs(model)
    phi_model = np.angle(model)
    return amp_model, phi_model, Css_real, freqProj_iB, Cps, transmission_ratio
...
if not os.path.isdir(path_output + 'astrometry_figures/amp_osc_model_allframes'):
    os.makedirs(path_output + 'astrometry_figures/amp_osc_model_allframes')
...
if not os.path.isdir(path_output + 'astrometry_figures/real_osc_model_allframes'):
    os.makedirs(path_output + 'astrometry_figures/real_osc_model_allframes')
...
print('BEST ASTROMETRY:', xp_allframe,yp_allframe, 'INDEX:',idx_allframe,idy_allframe, 'chi2 map all frame shape:', chi2_map_allframe.shape)
...
files_planet = sorted([file for file in results_files if 'planet' in file and 'flagged' not in file and 'antiplanet' not in file])

# Search star OBs 
files_star = sorted([file for file in oifits_files if 'star' in file and 'flagged' not in file])

# Find all the available OB numbers
OB_list = set([file[file.find('OB')+2:file.find('_group')] for file in files_planet])
...
for i, file in enumerate(files_planet):
    ...
    i_OB = OB_association_number(OB_list_star, i_OB)
    print('i_OB_star:', i_OB)
    ...
    # print('cf_phi - cf_phi_star',cf_phi_cal)
    # print('cf_amp / cf_amp_star', cf_amp_cal)
    ...
    # Complexify and take the real and imag part
    cf_full = cf_amp_cal * np.exp(1j * cf_phi_cal)
    cf_real_cal = np.real(cf_full)
    cf_imag_cal = np.imag(cf_full)
    # print('CF real and imag', cf_real_cal, cf_imag_cal)
    ...
    print('AMP AND ERROR:', cf_amp_cal_err)
    ...
    fig_allframe_osc, axes_allframe_osc = plt.subplots(6, 1, figsize=(6, 8))
    axes_allframe_osc = axes_allframe_osc.flat
    ...
    fig_real_allframe_osc, axes_real_allframe_osc = plt.subplots(6, 1, figsize=(6, 8))
    axes_real_allframe_osc = axes_real_allframe_osc.flat
    ...
    for iB in range(n_base):
        ...
        # fac = params_best[1]
        # stellar_residuals_poly_coeffs = params_best[2:]
        ...
        # print('PRINTS:', Cps)
        print('SHAPES FAC STELLAR COEFS:', transmission_ratio.shape, transmission_ratio, stellar_residuals_poly_coeffs.shape, stellar_residuals_poly_coeffs)
        amp_model, phi_model = model(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj[iB])
        
        # Plot the model and the data
        # print('amp model',amp_model)
        # Amplitudes 
        ...
        # HR8799?
        # axes[iB].set_ylim(0, 0.008)
        # betaPic b
        # axes[iB].set_ylim(0, 0.002)
        #HD72946B
        axes[iB].set_ylim(0, 0.03)
        ...
        # fac = params_best_allbase[1]
        # stellar_residuals_poly_coeffs = params_best_allbase[2:]
        ...
        # HR8799
        # axes_allbase[iB].set_ylim(0, 0.008)
        # betaPicb
        # axes_allbase[iB].set_ylim(0, 0.002)
        # HD72946B
        axes_allbase[iB].set_ylim(0, 0.03)
        ...
        # cf_model = contrast_model + R_model * np.exp(2j * np.pi * (offset_RA*U[iB] + offset_Dec*V[iB]) /(wl*1e-6)) 
        # cf_model = contrast_model * np.exp(-2j * np.pi * (offset_RA*U[iB] + offset_Dec*V[iB]) /(wl*1e-6)) + R_model 
        # print('MAX real model',np.max(np.real(cf_model)))
        ...
        # fac = params_best_allframe[1]
        # stellar_residuals_poly_coeffs = params_best_allframe[2:]
        stellar_residuals_poly_coeffs = params_best_allframe[1:]
        #stellar_residuals_poly_coeffs = params_best_allframe
        
        # Compute the whole model
        amp_model, phi_model, Css_real, freqProj_iB, Cps, transmission_ratio = model2(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_allframe[iB])
        ...
        # axes_allframe[iB].plot(wl, np.abs(cf_model), '-', c='chartreuse')
        ...
        # HD72946B
        axes_allframe[iB].set_ylim(-0.002, 0.02)
        ...
        axes_allframe_osc[iB].plot(wl, cf_amp_cal[iB]**2 - Css_real**2 - Cps**2, '+', markersize=5, c='tomato')
        # axes_allframe_osc[iB].fill_between(wl, cf_amp_cal[iB] - cf_amp_cal_err[iB] - Css_real**2 - Cps**2, cf_amp_cal[iB] + cf_amp_cal_err[iB] - Css_real**2 - Cps**2,  color='tomato', alpha=0.2, zorder=-100)
        axes_allframe_osc[iB].plot(wl, amp_model**2 - Css_real**2 - Cps**2, c='purple')
        # axes_allframe_osc[iB].plot(wl,  2*Css_real*Cps*np.cos(2*np.pi*freqProj_iB), c='chartreuse')
        # print('amp mod model', transmission_ratio*2*Css_real*Cps*np.cos(2*np.pi*freqProj_iB))

        # HD72946B
        axes_allframe_osc[iB].set_ylim(-0.00003, 0.00004)
        axes_allframe_osc[iB].set_ylabel(base_name[iB])
        axes_allframe_osc[iB].spines['top'].set_visible(False)
        axes_allframe_osc[iB].spines['right'].set_visible(False)
        if iB != 5:
            axes_allframe_osc[iB].set_xticklabels([])
        ...
        axes_phi_allframe[iB].set_ylim(-10, 10)
        axes_phi_allframe[iB].set_ylabel(base_name[iB])
        axes_phi_allframe[iB].set_yticks([-20, 0, 20], ['-20°', '0', '20°'])
        ...
        # HD72946
        axes_real_allframe[iB].set_ylim(0, 0.03)
        ...
        axes_real_allframe_osc[iB].plot(wl, cf_real_cal[iB] - Css_real, '+', markersize=5, c='tomato')
        axes_real_allframe_osc[iB].fill_between(wl, cf_real_cal[iB]-cf_real_cal_err[iB]- Css_real, cf_real_cal[iB]+cf_real_cal_err[iB]- Css_real,  color='tomato', alpha=0.2, zorder=-100)
        axes_real_allframe_osc[iB].plot(wl, real_model - Css_real, c='purple')
        # HD72946
        axes_real_allframe_osc[iB].set_ylim(-0.003, 0.003)
        axes_real_allframe_osc[iB].set_ylabel(base_name[iB])
        axes_real_allframe_osc[iB].spines['top'].set_visible(False)
        axes_real_allframe_osc[iB].spines['right'].set_visible(False)
        if iB != 5:
            axes_real_allframe_osc[iB].set_xticklabels([])

        # Imaginary parts
        axes_imag_allframe[iB].plot(wl, cf_imag_cal[iB], '+', markersize=5, c='tomato')
        # axes_imag_allframe[iB].plot(wl, np.imag(cf_model), '-', c='chartreuse')
        ...
        axes_imag_allframe[iB].plot(wl, 0.001*np.sin(2*np.pi*freqProj_iB+np.pi/2), c='purple', alpha=0.5)
        # HD72946
        axes_imag_allframe[iB].set_ylim(-0.006, 0.006)
    ...
    axes_allframe_osc[-1].set_xlabel(r'Wavelength [$\mu$m]')
    fig_allframe_osc.suptitle('Amplitude modulation of the coherent flux ratio')
    fig_allframe_osc.tight_layout()
    fig_allframe_osc.savefig(path_output + f'/astrometry_figures/amp_osc_model_allframes/{file}_amp_osc_model_allframes.pdf')
    ...
    axes_real_allframe_osc[-1].set_xlabel(r'Wavelength [$\mu$m]')
    fig_real_allframe_osc.suptitle('Real part modulation of the coherent flux ratio')
    fig_real_allframe_osc.tight_layout()
    fig_real_allframe_osc.savefig(path_output + f'/astrometry_figures/real_osc_model_allframes/{file}_real_osc_model_allframes.pdf')
