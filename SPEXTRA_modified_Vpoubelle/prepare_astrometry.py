...
from common_tools import reorder_baselines, wrap, get_DIT, OB_association_number
...
def compute_average_star_quantities(path_oifits, filelist_star):
    ...
    for i_file, file in enumerate(filelist_star):
        ...
        print('sta index',hdul['OI_VIS'].data['STA_INDEX'])
        ...
        cf_amp_star_all[i_file]     *= (DIT_planet/DIT_star[0])
        cf_amp_star_err_all[i_file] = hdul['OI_VIS'].data['VISAMPERR']
        cf_amp_star_err_all[i_file] *= (DIT_planet/DIT_star[0])
    ...
    # mean_cf_amp_star = np.mean(cf_amp_star_all, axis=0)
    # mean_cf_amp_star_err = np.mean(cf_amp_star_err_all, axis=0)
    # mean_cf_phi_star = np.mean(cf_phi_star_all, axis=0)
    # mean_cf_phi_star_err = np.mean(cf_phi_star_err_all, axis=0)

    print('MEAN STAR ERRORS',mean_cf_amp_star_err, mean_cf_phi_star_err)
...
if __name__ == '__main__':
    ...
    print(OB_list_planet, OB_list_star)
