...
def model(params, wl, freqProj, Cps):
    ...
    # stellar_residuals_poly_coeffs_A = params[1]
    # stellar_residuals_poly_coeffs_B = params[2]
    ...
    cf_model = trans_ratio * Cps * (np.cos(2 * np.pi * freqProj) + 1j * np.sin(2 * np.pi * freqProj)) + Css_real #+ stellar_residuals_poly_coeffs_A * wl + stellar_residuals_poly_coeffs_B
...
def residuals(params, wl, freqProj, Cps, real_err_mask, imag_err_mask, cf_real, cf_real_err, cf_imag, cf_imag_err):
    ...
    # stellar_residuals_poly_coeffs_A = params[1]
    # stellar_residuals_poly_coeffs_B = params[2]
    # cf_model = trans_ratio * Cps * (np.cos(2 * np.pi * freqProj) + 1j * np.sin(2 * np.pi * freqProj)) + stellar_residuals_poly_coeffs_A * wl + stellar_residuals_poly_coeffs_B
    # amp_model = np.abs(cf_model)
    # phi_model = np.angle(cf_model)
...
def residuals_full_output(params, wl, freqProj, Cps, real_err_mask, imag_err_mask, cf_real, cf_real_err, cf_imag, cf_imag_err):
    # Model
    amp_model, phi_model = model(params, wl, freqProj, params[1] * Cps)
...
# Update the fit function to include 'fac' in the arguments
def fit(ix, iy, Bcov, PAcov, pa, sep, wl, n_base, params_init, Cps, real_err_mask, imag_err_mask, 
        cf_real, cf_real_err, cf_imag, cf_imag_err, bounds):
    # Compute projected spatial frequencies
    Bproj = Bcov * mas2rad(sep[iy, ix]) * np.cos(PAcov - pa[iy, ix])  # scalar product planet vector - baseline vector
    ...
    for iB in range(n_base):
        ...
        # Save the fitted parameters
        fit_params_xy[iB] = res.x
        # with open(path_output + '/log.txt', 'a') as params_txt:
        #     params_txt.write(f"res.x = {res.x}\n")
...
if __name__ == '__main__':
    ...
    # stellar_residuals_poly_coeffs_init = (1.32557020e-05,  2.61251921e-05, -6.78750772e-05,  1.76543933e-05) * 2
    stellar_residuals_poly_coeffs_init = [1e-2, 4e-2] #* (n_poly+1) 
    # stellar_residuals_poly_coeffs_init_A = 1e-2
    # stellar_residuals_poly_coeffs_init_B = -0.04
    # stellar_residuals_poly_coeffs_init = [4.17213216e-04,  1.13739993e-03,  4.02701826e-06, -1.17941832e-06]
    # stellar_residuals_poly_coeffs_init = [3.23662920e-04,  8.44241889e-04,  4.02555742e-06, -1.18630583e-06]
    ...
    fac_init = [1.0]  # Initial multiplicative factor for the contrast
    ...
    i_OB = int(file[file.find('OB')+2:file.find('_group')])
    
    with open(path_output + '/log.txt', 'w') as params_txt:
        params_txt.write(f"i_OB={i_OB}\n")
        params_txt.write(f'Cps shape: {Cps.shape}\n')
    ...
    OB_list_planet = set([int(file[file.find('OB')+2:file.find('_group')]) for file in files if '_planet' in file])
    OB_list_star = set([int(file[file.find('OB')+2:file.find('_group')]) for file in files if '_star' in file])
    ...
    with open(path_output + '/log.txt', 'a') as params_txt:
        params_txt.write(f"i_OB_star={i_OB_star}\n")


    # Load the stellar average quantities for this OB
    cf_amp_star_data = fits.getdata(path_output + f'/stellar_OB_averages/star_avg_visamp_OB{i_OB_star}.fits')
    cf_amp_star, cf_amp_err_star = cf_amp_star_data[0], cf_amp_star_data[1]
    cf_phi_star_data = fits.getdata(path_output + f'/stellar_OB_averages/star_avg_visphi_OB{i_OB_star}.fits')
    ...
    with open(path_output + '/log.txt', 'a') as log_txt:
        log_txt.write(f'Real error: {cf_real_cal_err}\n')
        log_txt.write(f'Imag error: {cf_imag_cal_err}\n')
    ...
    # params_init = np.array([transmission_ratio_init, *stellar_residuals_poly_coeffs_init])  #*fac_init, Include 'fac' with initial guess 1.0
    params_init = np.array([transmission_ratio_init, *stellar_residuals_poly_coeffs_init])#stellar_residuals_poly_coeffs_init_A, stellar_residuals_poly_coeffs_init_B]) 
    n_params = params_init.size
    # bounds = [(0, None), *[(None, None)] * (n_poly + 1)]  # Add bounds for 'fac' (non-negative) *[(0.1, None)]
    bounds = [(0, None), *[(-0.1, 0.1)]*2] #(-0.1, 0.1), (-0.1, 0.1)] 
    # params_init = np.array([transmission_ratio_init, *stellar_residuals_poly_coeffs_init])
    # n_params = params_init.size
    # bounds = [(0, None), *[(-0.006, 0.006)]*(n_poly+1)*2]
    ...
    # cf_real_cal_err = 1e-3 * np.ones_like(real_err_mask)
    # cf_real_cal_err = 1e-4 * np.ones_like(imag_err_mask)
    ...
    with open(path_output + '/log.txt', 'a') as log_txt:
        log_txt.write(f'Factor of the Cps {params[1:]}\n')
    ...
    with open(path_output + '/.txt', 'a') as params_txt:
