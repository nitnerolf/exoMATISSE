...
path_oifits = '/data/home/jscigliuto/Pipeline/HD72946B/corrected_data_stacked_exposures_2m/'
...
files = [f for f in os.listdir(path_oifits_corrected) if f.endswith(".fits")]
...
for file in files:
    ...
    cf_phi     = hdul['OI_VIS'].data['VISPHI']
    cf_phi_err = hdul['OI_VIS'].data['VISPHIERR']
    ...
    cf_amp = cf_amp.reshape(n_base, -1, 5).mean(axis=2)
    cf_phi = cf_phi.reshape(n_base, -1, 5).mean(axis=2)
    cf_phi_err = cf_phi_err.reshape(n_base, -1, 5).mean(axis=2)
    cf_amp_err = cf_amp_err.reshape(n_base, -1, 5).mean(axis=2)

    print('erreurs amp / phi', cf_amp_err, cf_phi_err)
    ...
    t3_amp = t3_amp.reshape(n_triangle, -1, 5).mean(axis=2)
    t3_phi = t3_phi.reshape(n_triangle, -1, 5).mean(axis=2)
    t3_amp_err = t3_amp_err.reshape(n_triangle, -1, 5).mean(axis=2)
    t3_phi_err = t3_phi_err.reshape(n_triangle, -1, 5).mean(axis=2)
    ...
    visphi_col = fits.Column(name='VISPHI', array=cf_phi, unit='deg', format=f'{n_wave}D')
    visamperr_col = fits.Column(name='VISAMPERR', array=cf_amp_err, format=f'{n_wave}D')
    visphierr_col = fits.Column(name='VISPHIERR', array=cf_phi_err, unit='deg', format=f'{n_wave}D')
