...
path_oifits = '/data/home/jscigliuto/Pipeline/HD72946B/corrected_data_stacked_exposures_15m/corrected_data_5bin/'

# Output directory
path_output = '/data/home/jscigliuto/Pipeline/HD72946B/Result2_without_rmnrec_bin_stacked_test/'
...
#Coherent flux model
contrast_model = 2.258871822415009e-05
...
def model2(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_iB):
    ...
    amp_model = np.abs(model)
    phi_model = np.angle(model)
    return amp_model, phi_model, Css_real, freqProj_iB
...
# print('OB LIST STAR:', OB_list_star)
# Load the stellar average quantities
stellar_averages = {}
for i_OB in OB_list_star:
    # print('ob list star:', OB_list_star)
    # i_OB = int(i_OB)
    # i_OB_star = OB_association_number(OB_list_star, i_OB)
    # print('i_OB_star:', i_OB_star)
...
for i, file in enumerate(files_planet):
    ...
    # fac = params[..., 1]
    # stellar_residuals_poly_coeffs = params[..., 2:]
    ...
    for iB in range(n_base):
        ...
        # amp_model, phi_model, Css_real = model2(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_allbase[iB])
        ...
        # Compute the whole model
        amp_model, phi_model, Css_real, freqProj_iB = model2(wl, transmission_ratio, stellar_residuals_poly_coeffs, Cps, freqProj_allframe[iB])
        ...
        # axes_allframe[iB].plot(wl, amp_model, c='chartreuse')
        # HR8799
        # axes_allframe[iB].set_ylim(-0.002, 0.008)
        # betaPicb
        # axes_allframe[iB].set_ylim(-0.002, 0.003)
        # HD72946B
        axes_allframe[iB].set_ylim(-0.002, 0.003)
        ...
        # axes_phi_allframe[iB].plot(wl, np.rad2deg(wrap(np.angle(cf_model))), '-', c='chartreuse')
        ...
        # axes_allframe[iB].plot(wl, cf_amp_cal[iB], '.', c='tomato')
        # axes_allframe[iB].plot(wl, cf_amp_cal[iB], '.', markersize=5, c='tomato')
        axes_real_allframe[iB].plot(wl, cf_real_cal[iB]-Css_real, '+', markersize=5, c='tomato')
        # axes_real_allframe[iB].plot(wl, np.real(cf_model), '-', c='chartreuse')
        # print(cf_real_cal_err)
        ...
        # axes_real_allframe[iB].plot(wl, real_model-Css_real, c='purple')
        axes_real_allframe[iB].plot(wl, np.real(0.001*(1j*np.sin(2*np.pi*freqProj_iB*10)+np.cos(2*np.pi*freqProj_iB*10))) , c='purple')
        # HR8799
        # axes_real_allframe[iB].set_ylim(-0.004, 0.008)
        # betaPic
        # axes_real_allframe[iB].set_ylim(-0.001, 0.003)
        # HD72946
        # axes_real_allframe[iB].set_ylim(-2e-3, 2e-3)
        axes_real_allframe[iB].set_ylim(-0.2, 0.2)
        ...
        # HR8799
        # axes_imag_allframe[iB].set_ylim(-0.004, 0.008)
        # betaPic
        # axes_imag_allframe[iB].set_ylim(-0.001, 0.003)
        # HD72946
        axes_imag_allframe[iB].set_ylim(-0.002, 0.002)
