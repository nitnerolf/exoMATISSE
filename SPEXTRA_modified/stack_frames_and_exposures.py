...
mode = 'average'
n_exposures_to_stack = 2

path = '/data/home/jscigliuto/Pipeline/HD72946B/corrPhase_without_rmnrec/'

path_frames = path + '/corrected_data/'
path_frames_stacked = '/data/home/jscigliuto/Pipeline/HD72946B' + '/corrected_data_stacked_exposures_2m/'
...
for i_OB in all_ob_numbers:
    ...
    all_exp_numbers_of_OB = set(sorted([int(f[f.find('exp')+3:f.find('_frame')]) for f in all_frames if ('OB' + str(i_OB) + '_') in f and 'flagged' not in f]))
    #print(all_exp_numbers_of_OB)

    n_groups = len(all_exp_numbers_of_OB) // n_exposures_to_stack
    if len(all_exp_numbers_of_OB) % n_exposures_to_stack != 0:
        #print(f'Number of exposures in OB {str(i_OB)} ({len(all_exp_numbers_of_OB)}) is not a multiple of 4.')
        #print(f'Last group will stack only {len(all_exp_numbers_of_OB) % n_exposures_to_stack} exposures.')
        n_groups += 1

    for i_group in range(n_groups):

        all_exp_numbers_of_group = np.arange(i_group * n_exposures_to_stack + 1, min((i_group+1) * n_exposures_to_stack + 1, len(all_exp_numbers_of_OB)+1))
        all_frames_group = []

        for i_exp in all_exp_numbers_of_group:
            # Get all frames
            all_frames_exp = sorted([f for f in all_frames if ('OB' + str(i_OB) + '_exp' + str(i_exp) + '_') in f and 'flagged' not in f])
            for f in all_frames_exp:
                all_frames_group.append(f)

        print('Stacking OB ' + str(i_OB) + ' exposure ' + str(all_exp_numbers_of_group) + ' with ' + str(len(all_frames_group)) + ' frames.')
        
        ## Create the stacked OIFITS

        # Copy some extensions from the RAW_VIS2 files
        hdul_ref = fits.open(path_frames + all_frames_group[0])
        ...
        if mode == 'stack':
            ...
            # visdata = np.zeros((len(all_frames_group), 6, n_wave), dtype=complex)
            # viserr = np.zeros((len(all_frames_group), 6, n_wave), dtype=complex)

            visamp = np.zeros((len(all_frames_group), 6, n_wave))
            visamperr = np.zeros((len(all_frames_group), 6, n_wave))
            visphi = np.zeros((len(all_frames_group), 6, n_wave))
            visphierr = np.zeros((len(all_frames_group), 6, n_wave))

            ucoord = np.zeros((len(all_frames_group), 6))
            vcoord = np.zeros((len(all_frames_group), 6))
            mjd = np.zeros((len(all_frames_group), 6))
            inttime = np.zeros((len(all_frames_group), 6))
            staindex = np.zeros((len(all_frames_group), 6, 2), dtype=int)
            targetid = np.zeros((len(all_frames_group), 6), dtype=int)

            t3_amp = np.zeros((len(all_frames_group), 4, n_wave))
            t3_phi = np.zeros((len(all_frames_group), 4, n_wave))
            t3_amp_err = np.zeros((len(all_frames_group), 4, n_wave))
            t3_phi_err = np.zeros((len(all_frames_group), 4, n_wave))
            u1coord = np.zeros((len(all_frames_group), 4))
            u2coord = np.zeros((len(all_frames_group), 4))
            v1coord = np.zeros((len(all_frames_group), 4))
            v2coord = np.zeros((len(all_frames_group), 4))
            mjd_t3 = np.zeros((len(all_frames_group), 4))
            inttime_t3 = np.zeros((len(all_frames_group), 4))
            staindex_t3 = np.zeros((len(all_frames_group), 4, 3), dtype=int)
            targetid_t3 = np.zeros((len(all_frames_group), 4), dtype=int)

            #exograv_visref = np.zeros((len(all_frames_group), 6, n_wave), dtype=complex)


        for i_frame, frame in enumerate(all_frames_group):
        ...
        if mode == 'average':
            ...
            cf_full_real_err = np.sqrt((np.cos(visphi) * visamperr) ** 2 + (visamp * np.sin(visphi) * visphierr) ** 2)
            cf_full_imag_err = np.sqrt((np.sin(visphi) * visamperr) ** 2 + (visamp * np.cos(visphi) * visphierr) ** 2)

            #Average in the complex space 
            mean_cf_full = np.mean(np.real(cf_full), axis=0) + 1j * np.mean(np.imag(cf_full), axis=0)
            mean_cf_full_real_err = np.sqrt(np.sum(cf_full_real_err**2, axis=0)) / cf_full_real_err.shape[0]
            mean_cf_full_imag_err = np.sqrt(np.sum(cf_full_imag_err**2, axis=0)) / cf_full_imag_err.shape[0]#len(all_frames_group)

            print('SIZE',cf_full_real_err.shape)

            #Go back to amp/phase space
            visamp = np.abs(mean_cf_full)
            visphi = wrap(np.angle(mean_cf_full))
            # visphi = np.angle(mean_cf_full)
            visamperr = np.sqrt((np.real(mean_cf_full) * mean_cf_full_real_err) ** 2 + (np.imag(mean_cf_full) * mean_cf_full_imag_err) ** 2) / visamp
            visphierr = np.sqrt((np.imag(mean_cf_full) * mean_cf_full_real_err) ** 2 + (np.real(mean_cf_full) * mean_cf_full_imag_err) ** 2) /visamp**2
            # visphierr = np.sqrt((np.imag(mean_cf_full) * mean_cf_full_real_err) ** 2 + (np.real(mean_cf_full) * mean_cf_full_imag_err) ** 2) /visphi**2
            
            print('VISAMPERR / VISPHIERR', visamperr, '\n', visphierr)
            ...
            #exograv_visref = np.mean(np.abs(exograv_visref), axis=0) * np.exp(1j * np.angle(np.mean(np.exp(1j*np.angle(exograv_visref)), axis=0)))
        ...
        # visamp_col = fits.Column(name='VISAMP', array=np.abs(visdata), format=f'{n_wave}D')
        # visphi_col = fits.Column(name='VISPHI', array=np.angle(visdata, deg=True), unit='deg', format=f'{n_wave}D')
        ...
        hdu_vis = fits.BinTableHDU.from_columns([targetid_col, mjd_col, inttime_col, visamp_col, visamperr_col, 
                                                 visphi_col, visphierr_col, ucoord_col, vcoord_col, staindex_col], 
                                                 name='OI_VIS', header=hdul_ref['OI_VIS'].header) #visdata_col, viserr_col,
        ...
        date_obs = Path(all_frames_group[0]).stem[:all_frames_group[0].find('_OB')]
        target = Path(all_frames_group[0]).stem.split('_')[-1]
        hdul_stacked.writeto(f"{path_frames_stacked}/{date_obs}_OB{i_OB}_group{i_group+1}_{target}.fits", overwrite=True)
