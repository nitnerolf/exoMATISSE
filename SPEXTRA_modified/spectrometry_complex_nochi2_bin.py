...
from common_tools import wrap, reorder_baselines, mas2rad, OB_association_number
...
#path_oifits = './postprocessing/results_20230913_noUVbcdsign_guessFromPrevFit/corrected_data_bin/'
path_oifits = '/data/home/jscigliuto/Pipeline/HD72946B/corrected_data_stacked_exposures_2m/corrected_data_5bin/'
#path_oifits = './OIFITS_corrected_newpipe_vfactor/'

# Output results directory
#path_output = './results_20230911_mp_errdata_snrmask_modelcontrast/'
path_output = '/data/home/jscigliuto/Pipeline/HD72946B/Result2_without_rmnrec_bin_2m_stacked/'
...
# x = np.arange(280-10, 280+10.4, 0.4) # mas
# y = np.arange(455-10, 455+10.4, 0.4) # mas
# x = np.arange(280-10, 280+10.1, 0.1) # mas
# y = np.arange(455-10, 455+10.1, 0.1) # mas
# x = np.arange(293-15, 293+15, 0.4) # mas
# y = np.arange(469-15, 469+15, 0.4) # mas 
# beta Pic b MACAO
...
# y = np.arange(469-15, 469+15, 0.4) # mas 
# HD 72946 B
x = np.arange(107-20, 107+20, 0.4) # mas
y = np.arange(-146-20, -146+20, 0.4) # mas  
...
# If False, output standard deviation vector
...
print('INDEX BEST ASTROMETRY:', idx, idy)

# Find all the available OB numbers
##########################################################################################################################################
###############################################STACKED FRAMES + EXPOSURE##################################################################
##########################################################################################################################################

# OB_list = set([file[file.find('OB')+2:file.find('_exp')] for file in files])
# OB_list_star = set([int(file[file.find('OB')+2:file.find('_exp')]) for file in files_star])

OB_list = set([file[file.find('OB')+2:file.find('_group')] for file in files])
OB_list_star = set([int(file[file.find('OB')+2:file.find('_group')]) for file in files_star])
...
for i, file in enumerate(files):
    ...
    try:
        ...
        print(f'Fichier manquant pour {file}, on passe au suivant.')
        continue
    
    except OSError as e:
        print(f'Erreur d’ouverture FITS pour {file} : {e}, on passe au suivant.')
        continue
    ...
    i_OB = file[file.find('OB')+2:file.find('_group')]

    ##########################################################################################################################################
    ###############################################STACKED FRAMES + EXPOSURE##################################################################
    ##########################################################################################################################################

    i_OB = OB_association_number(OB_list_star, int(i_OB))
    ...
    #modified
    # transmission_all = np.concatenate((transmission_all, params[:, idx, idy, 0] * params[:, idx, idy, 1]))
    # stellar_coeffs_all = np.concatenate((stellar_coeffs_all, params[:, idx, idy, 2:]))
    #previous 
...
# contrast_err = np.std(Cps, axis=0, ddof=1) / np.sqrt(Cps.shape[0])
# ##contrast_err = np.sqrt(np.sum(cf_real_err_all**2, axis=0)) / Cps.shape[0]
# contrast_cov = np.cov(Cps.T, ddof=1) / Cps.shape[0]
...
#     low_limit = np.quantile(Cps[:, iw], 0.05)
#     high_limit = np.quantile(Cps[:, iw], 0.95)
...
# Method 5bis: mean with MAD outlier rejection
contrast = np.zeros_like(wl)
contrast_err = np.zeros_like(wl)
contrast_err_nonorm = np.zeros_like(wl)
contrast_cov = np.zeros((wl.size, wl.size))
Cps_valid_all = np.zeros_like(Cps, dtype=bool)
plt.figure()
for iw in range(wl.size):
    median_Cps = np.median(Cps[:, iw])
    std_Cps = 3 * 1.4826 * np.median(np.abs(Cps[:, iw] - median_Cps))
    Cps_valid = (Cps[:, iw] > median_Cps - std_Cps) & (Cps[:, iw] < median_Cps + std_Cps)
    Cps_valid_all[:, iw] = Cps_valid
    contrast[iw] = np.mean(Cps[Cps_valid, iw])
    contrast_err[iw] = np.std(Cps[Cps_valid, iw], ddof=1) / np.sqrt(Cps_valid.sum())
    plt.semilogy(wl[iw], 100*(Cps[:, iw].size - Cps_valid.sum())/Cps[:, iw].size, 'b+')
    plt.semilogy(wl[iw], np.median(Cps[Cps_valid, iw] / cf_real_err_all[Cps_valid, iw]), 'r+')
    plt.semilogy(wl[iw], np.median(Cps[Cps_valid, iw] / (contrast_err[iw]*np.sqrt(Cps_valid.sum()))), 'g+')
plt.semilogy([], [], 'b+', label='Outlier fraction [%]')
plt.semilogy([], [], 'r+', label='Median S/N with pipeline errors')
plt.semilogy([], [], 'g+', label='Median S/N with inferred errors')
plt.legend()
plt.xlabel(r'Wavelength [$\mu$m]')
# Compute covariance matrix of valid data taking into account the mask
# Cps_masked = Cps.copy()
# Cps_masked[~Cps_valid_all] = 0
for iw1 in range(wl.size):
    for iw2 in range(wl.size):
        combined_mask = Cps_valid_all[:, iw1] & Cps_valid_all[:, iw2]
        mean_w1 = np.mean(Cps[combined_mask, iw1])
        mean_w2 = np.mean(Cps[combined_mask, iw2])
        contrast_cov[iw1, iw2] = np.sum((Cps[combined_mask, iw1]-mean_w1) * (Cps[combined_mask, iw2]-mean_w2)) \
                                 / (combined_mask.sum()*(combined_mask.sum()-1))
# for iw in range(wl.size):
#     Cps_valid_all_iw = Cps_valid_all.copy()
#     Cps_valid_all_iw[~Cps_valid_all[:, iw], :] = False
#     N_Cps_valid_all_iw = Cps_valid_all_iw.sum(axis=0)
#     Cps_masked_iw = Cps_masked.copy()
#     Cps_masked_iw[~Cps_valid_all_iw] = 0
#     Cps_masked_iw_centered = Cps_masked_iw - Cps_masked_iw.sum(axis=0) / N_Cps_valid_all_iw
#     contrast_cov[:, iw] = np.squeeze(Cps_masked_iw_centered.T @ Cps_masked_iw_centered[:, iw][:, np.newaxis]) / (N_Cps_valid_all_iw*(N_Cps_valid_all_iw - 1))
plt.savefig(path_output + '/spectrometry/data_selection.pdf')
...
#     Cps_valid = weighted_Cps[:, iw] < high_limit
...
#chi2r_newerr = np.mean(((Cps - contrast) / contrast_err_nonorm) ** 2, axis=0)
plt.figure()
plt.semilogy(wl, chi2r, label='Chi2 pipeline')
#plt.semilogy(wl, chi2r_newerr, label='Chi2 data')
plt.semilogy(wl, np.sqrt(chi2r), label='sqrt(Chi2) pipeline')
#plt.semilogy(wl, np.sqrt(chi2r_newerr), label='sqrt(Chi2) data')
plt.legend()
plt.savefig(path_output + '/spectrometry/chi2_contrast.png')

# corr_Cps = np.corrcoef(Cps.T)
# plt.figure()
# plt.imshow(corr_Cps, cmap='jet')
# plt.colorbar()
# plt.savefig(path_output + '/spectrometry/corr_Cps.pdf')

plt.figure()
plt.imshow(np.log10(np.abs(contrast_cov)), cmap='jet')
plt.colorbar()
plt.savefig(path_output + '/spectrometry/cov_Cps_outlier.pdf')

# plt.figure()
# plt.imshow((contrast_cov-contrast_cov.T)/contrast_cov, cmap='jet')
# plt.colorbar()
# plt.savefig(path_output + '/spectrometry/cov_Cps_outlier_residuals.pdf')

contrast_var = np.diag(contrast_cov)
var_product = np.outer(contrast_var, contrast_var)
contrast_corr = contrast_cov / np.sqrt(var_product)
#print(contrast_corr.max(), contrast_corr.min())

plt.figure()
plt.imshow(contrast_corr, cmap='jet')#, vmin=-1, vmax=1)
plt.colorbar()
plt.savefig(path_output + '/spectrometry/corr_Cps_outlier.pdf')

plt.figure()
plt.semilogy(wl, contrast_err, label='Errors from standard deviation')
plt.semilogy(wl, np.sqrt(contrast_var), '--', label='Errors from covariance matrix diagonal')
plt.savefig(path_output + '/spectrometry/contrast_errors_comparison.pdf')
...
plt.imshow(cov_Cps_wlbase, cmap='bwr')
plt.colorbar()
...
np.savetxt(path_output + '/spectrometry/snr.txt', np.array([wl[::-1], snr[::-1]]).T)

print(f'Median contrast (3-4.0 um): {np.median(contrast[(wl>3.0)&(wl<4.0)])}')
print(f'Median contrast (4.6-5.0 um): {np.median(contrast[(wl>4.6)&(wl<5.0)])}')

print(f'S/N: median (3-4.0 um): {np.median(snr[(wl>3.0)&(wl<4.0)])}, mean: {np.mean(snr[(wl>3.0)&(wl<4.0)])},\
      max: {np.max(snr[(wl>3.0)&(wl<4.0)])}')

print(f'S/N: median (4.6-5.0 um): {np.median(snr[(wl>4.6)&(wl<5.0)])}, mean: {np.mean(snr[(wl>4.6)&(wl<5.0)])},\
      max: {np.max(snr[(wl>4.6)&(wl<5.0)])}')
...
ax.errorbar(wl, contrast, yerr=contrast_err, label='MATISSE contrast', color='snow')
#ax.plot(wl, contrast, '+', c='tab:red')
#ax.set_ylim(0, 0.001)
ax.set_xlabel(r'Wavelength [$\mu$m]', color='white')
ax.set_ylabel('Planet-to-star contrast', color='white')
#ax.set_ylim(1e-4, 12e-4)
#ax.set_ylim(3e-4, 12e-4)
ax.set_ylim(0, 10e-4)
#ax.set_xlim(2.9, 4.15)
ax.set_xlim(2.81, 4.1)

# Changer la couleur des axes et labels en blanc
ax.spines['bottom'].set_color('white')
ax.spines['left'].set_color('white')
ax.spines['top'].set_color('white')
ax.spines['right'].set_color('white')

# Couleur des ticks (graduations)
ax.tick_params(axis='x', colors='white')
ax.tick_params(axis='y', colors='white')

#resid = residuals(res.x, wl, freqProj[iB], sep_p, transmission_ratio, stellar_residuals_poly_coeffs, cf_ratio)

contrast_tpl = fits.getdata('/data/home/jscigliuto/Pipeline/Templates/HD72946/contrast_template_bt-settl_hd72946_ph1ld.fits')
contrast_tpl = contrast_tpl.reshape(-1, 5).mean(axis=1)
contrast_tpl = contrast_tpl[wl_mask]
# ax.plot(wl, contrast_tpl, zorder=100, label=r'Model: (BT-Settl, $T_{eff}$ = 1700 K, log(g) = 4.0) / (ISO Beta Pic spectrum)')

#ax.legend(loc='upper left')
# ax.legend(fontsize='small', loc='lower right')
fig.tight_layout()
fig.savefig(path_output + '/spectrometry/contrast_nochi2.png', transparent=True)

plt.figure(figsize=(15 , 5))
plt.plot(wl, contrast_tpl)
plt.savefig(path_output + '/spectrometry/contrast_tpl.png')

# Matthieu's template
file_star = np.loadtxt('/data/home/jscigliuto/Pipeline/Templates/HD72946/BT-NextGen_T5638K_lg4.51_M0.0_R9.09_res300.800.txt')
...
# file_star = np.loadtxt('archive_72501593_pws.txt')
...
#spec_star_conv = instrBroadGaussFast(w_star, spec_star, resolution=np.mean(wl/wband), equid=True)
...
# Compute the covariance matrix of the planetary spectrum
spec_planet_cov = contrast_cov * np.outer(spec_star_conv, spec_star_conv)


# Planetary spectrum model (BT-Settl / 1700 K / 4.0)
planet_tpl = fits.getdata('/data/home/jscigliuto/Pipeline/Templates/HD72946/planet_spectrum_template_bt-settl_hd72946_ph1ld.fits')
...
plt.figure(figsize=(15 , 5))
plt.plot(wl, planet_tpl)
plt.savefig(path_output + '/spectrometry/planet_tpl.png')
...
plt.figure(figsize=(15 , 5))
for i in range(6):
    #plt.plot(wl, (Cps[::6]*spec_star_conv).T*1e3, ms=0.1, marker='+', ls='', c='skyblue')
    plt.plot(wl, (Cps[i::6]*spec_star_conv).T*1e3, ms=0.3, marker='+', ls='', c=plt.cm.tab10.colors[i])
...
for i in range(6):
    #plt.plot(wl, (Cps[::6]*spec_star_conv).T*1e3, ms=0.1, marker='+', ls='', c='skyblue')
    axes[i].plot(wl, (Cps[i::6]*spec_star_conv).T*1e3, ms=0.5, marker='+', ls='', c=plt.cm.tab10.colors[i])
    axes[i].plot(wl, (Cps[i::6]*spec_star_conv*1e3).mean(axis=0), marker='+', ls='', c='red', zorder=100)
    axes[i].errorbar(wl, spec_planet*1e3, yerr=spec_planet_err*1e3, fmt='+', label='MATISSE spectrum', c='steelblue', zorder=100)
    axes[i].set_xlabel(r'Wavelength [$\mu$m]')
    axes[i].set_ylabel('Flux [mJy]')
    axes[i].legend(loc='upper left')
    axes[i].set_ylim(-10, 25)
    axes[i].set_xlim(2.76, 5.0)
fig.tight_layout()
fig.savefig(path_output + '/spectrometry/spectrum_planet_simple_nochi2_allspectra_baseline.png')
...
# Plot the planetary spectrum (updated uncertainties)
plt.figure(figsize=(15 , 5))
#plt.plot(wl, (Cps*spec_star_conv).T*1e3, ms=0.1, marker='+', ls='', c='skyblue')
plt.plot(wl, spec_planet*1e3, label='MATISSE spectrum', c='steelblue', zorder=100)
plt.fill_between(wl, (spec_planet-np.sqrt(chi2r)*spec_planet_err)*1e3, (spec_planet+np.sqrt(chi2r)*spec_planet_err)*1e3)
plt.xlabel(r'Wavelength [$\mu$m]')
plt.ylabel('Flux [mJy]')
plt.legend(loc='upper left')
plt.ylim(0, 15)
plt.xlim(2.76, 5.0)
plt.tight_layout()
plt.savefig(path_output + '/spectrometry/spectrum_planet_nochi2_filled_updatederrors.png')
...
# Plot the planet spectrum in SI W/m2/µm
...
pinhole_diameter = 1.5 * 3.5e-6 / 8 # rad
pixel_scale_xy = (3.2e-6 / 8) / 3 # rad/pix
pinhole_diameter_pix = pinhole_diameter / pixel_scale_xy
pixel_scale_wl = 1.4261867e-09 # m/pix
fwhm_wl = pixel_scale_wl * pinhole_diameter_pix
wres = (wl[::-1]*1e-6) / fwhm_wl
#wres = wl / wband

instrument_col = np.full(wl.size, 'MATISSE-LM-MED')

spec_planet_formosa = contrast[::-1] * spec_star_conv_SI
spec_planet_err_formosa = contrast_err[::-1] * spec_star_conv_SI
spec_planet_cov_formosa = contrast_cov[::-1, ::-1] * np.outer(spec_star_conv_SI, spec_star_conv_SI)

plt.figure()
plt.errorbar(wl[::-1], spec_planet_formosa, yerr=spec_planet_err_formosa)
plt.ylim(0, 1e-15)
plt.xlabel(r'Wavelength [$\mu$m]')
plt.ylabel('Flux [W/m2/µm]')
plt.savefig(path_output + '/spectrometry/spectrum_ForMoSA.png')

# Plot spec_planet_cov_formosa
plt.figure()
plt.imshow(spec_planet_cov_formosa, cmap='jet', vmin=1e-35, vmax=1e-32)
plt.colorbar()
plt.xlabel('Wavelength [µm]')
plt.ylabel('Wavelength [µm]')
plt.tight_layout()
plt.savefig(path_output + '/spectrometry/cov_spectrum_ForMoSA.png')
print(np.min(spec_planet_cov_formosa), np.max(spec_planet_cov_formosa))
print(np.min(spec_planet_cov_formosa[25:175, 25:175]), np.max(spec_planet_cov_formosa[25:175, 25:175]))
print(np.min(spec_planet_cov_formosa[250:, 250:]), np.max(spec_planet_cov_formosa[250:, 250:]))

# t = Table([wl[::-1], spec_planet_formosa[::-1]/1.15764592, spec_planet_err_formosa[::-1]/1.15764592, wres[::-1], instrument_col], names=('WAV', 'FLX', 'ERR', 'res', 'INS'))
# #t.write(path_output + 'spectrometry/spectrum_BetaPicb_formosa.fits', format='fits', overwrite=True)
# t.write(path_output + '/spectrometry/spectrum_BetaPicb_MATISSE-LM_rescaled.fits', format='fits', overwrite=True)

# With standard deviation
# t = Table([wl[::-1], spec_planet_formosa, spec_planet_err_formosa, wres, instrument_col], names=('WAV', 'FLX', 'ERR', 'res', 'INS'))
# t.write(path_output + '/spectrometry/spectrum_BetaPicb_formosa_MATISSE-LM_std.fits', format='fits', overwrite=True)
# t = Table([wl[::-1], contrast, contrast_err, wres, instrument_col], names=('WAV', 'CONTRAST', 'ERR', 'res', 'INS'))
# t.write(path_output + '/spectrometry/contrast_BetaPicb_formosa_MATISSE-LM_std.fits', format='fits', overwrite=True)

# With covariance matrix
wl_formosa = wl[::-1]
mask_wl = ((wl_formosa>2.9) & (wl_formosa<4.17)) | ((wl_formosa>4.5) & (wl_formosa<5.0))
spec_planet_cov_formosa = spec_planet_cov_formosa[mask_wl,:]
spec_planet_cov_formosa = spec_planet_cov_formosa[:,mask_wl]
t = Table([wl_formosa[mask_wl], spec_planet_formosa[mask_wl], spec_planet_cov_formosa, wres[mask_wl], instrument_col[mask_wl]], names=('WAV', 'FLX', 'COV', 'res', 'INS'))
t.write(path_output + '/spectrometry/spectrum_BetaPicb_formosa_MATISSE-LM_cov.fits', format='fits', overwrite=True)
t = Table([wl_formosa, contrast, contrast_cov, wres, instrument_col], names=('WAV', 'CONTRAST', 'COV', 'res', 'INS'))
t.write(path_output + '/spectrometry/contrast_BetaPicb_formosa_MATISSE-LM_cov.fits', format='fits', overwrite=True)
...
hdul_grav = fits.open('/data/home/jscigliuto/Pipeline/Templates/HD72946/spectra/HD72946B_GRAVITYK_fluxcal_spectrum_combined_final.fits')
...
contrast_cov_grav = hdul_grav['SPECTRUM'].data['COVARIANCE_CONTRAST'].data
contrast_err_grav = np.sqrt(np.diag(contrast_cov_grav))

# Matthieu Ravet's stellar template
file_star = np.loadtxt('/data/home/jscigliuto/Pipeline/Templates/HD72946/BT-NextGen_T5638K_lg4.51_M0.0_R9.09_res500.txt')
w_star, spec_star = file_star[:, 0], file_star[:, 1]
# print('star',w_star, spec_star)

#New way
# spec_star_grav = spectres.spectres(wl_grav, w_star, spec_star)
# spec_star_mat = spectres.spectres(wl[::-1], w_star, spec_star)

#Old way
f = interp1d(w_star, spec_star)
spec_star_grav = f(wl_grav)
spec_star_mat = f(wl[::-1])
...
# w_star_mat = w_star_mat.reshape(-1, 5).mean(axis=1)
# spec_star_mat = spec_star_mat.reshape(-1, 5).mean(axis=1)

spec_grav = contrast_grav * spec_star_grav
spec_err_grav = contrast_err_grav * spec_star_grav
spec_cov_grav = contrast_cov_grav * np.outer(spec_star_grav, spec_star_grav)

res_grav = 500 * np.ones_like(wl_grav)
ins_grav = np.full(wl_grav.size, 'GRAVITY-K-MED')

plt.figure(figsize=(10,5))
plt.plot(wl_grav, spec_grav, color = 'red')
plt.plot(wl, spec_planet_SI, color='blue')
plt.plot(wl, spec_planet_SI/4, color='green')
plt.ylim(-1e-15, 5e-15)
# plt.xlim(0, 6)
plt.xlabel(r'Wavelength [$\mu$m]')
plt.ylabel('Flux [W/m2/µm]')
plt.savefig(path_output + '/spectrometry/spectrum_MAT+GRAV.png')

#t = Table([wl_grav, spec_grav, spec_err_grav, res_grav, ins_grav], names=('WAV', 'FLX', 'ERR', 'res', 'INS'))
#t.write(path_output + '/spectrometry/spectrum_BetaPicb_formosa_GRAVITY_std.fits', format='fits', overwrite=True)

t = Table([wl_grav, spec_grav, spec_cov_grav, res_grav, ins_grav], names=('WAV', 'FLX', 'COV', 'res', 'INS'))
t.write(path_output + '/spectrometry/spectrum_BetaPicb_formosa_GRAVITY_cov.fits', format='fits', overwrite=True)
...
# t.write(path_output + '/spectrometry/spectrum_BetaPicb_formosa_GRAV+MAT.fits', format='fits', overwrite=True)

# Plot the stellar spectra
plt.figure()
plt.scatter(w_star, spec_star * 1e-26 * (cst.c.value / (w_star*1e-6)**2) * 1e-6, s=0.5, label='ISO spectrum')
plt.scatter(wl, spec_star_conv * 1e-26 * (cst.c.value / (wl*1e-6)**2) * 1e-6, s=0.5, c='tab:orange', label='Convolved ISO spectrum')
plt.scatter(wl[::-1], spec_star_mat, s=0.5, c='tab:green', label='Convolved model spectrum')
plt.xlim(3, 4)
plt.ylim(0.2e-11, 0.6e-11)
plt.legend()
plt.title('Stellar spectrum (ISO)')
plt.xlabel(r'Wavelength [$\mu$m]')
plt.ylabel('Flux [Jy]')
plt.savefig(path_output + '/spectrometry/spectrum_star.png')