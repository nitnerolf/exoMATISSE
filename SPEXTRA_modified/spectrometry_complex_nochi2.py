...
path_oifits = '/data/home/jscigliuto/Pipeline/HD72946B/corrPhase_stacked_Mathis/'

# Output results directory
#path_output = './results_20230911_mp_errdata_snrmask_modelcontrast/'
path_output = '/data/home/jscigliuto/Pipeline/HD72946B/Result2_without_rmnrec/'
...
# HR8799 MACAO
# x = np.arange(-223-15, -223+15, 0.4) # mas
# y = np.arange(332-15, 332+15, 0.4) # mas
# HR8799e GPAO
# x = np.arange(-192-15, -192+15, 0.4) # mas
# y = np.arange(353-15, 353+15, 0.4) # mas 
# beta Pic b GPAO
...
files = sorted([file for file in os.listdir(path_oifits) if 'planet' in file and 'flagged' not in file and 'antiplanet' not in file])
...
#     low_limit = np.quantile(Cps[:, iw], 0.01)
#     high_limit = np.quantile(Cps[:, iw], 0.98)
...
contrast_tpl = fits.getdata('/data/home/jscigliuto/Pipeline/Templates/HD72946/contrast_template_bt-settl_hd72946.fits')
if use_bin_data:
...
# Contrast plot masked S/N<5
snr_less_5 = contrast/contrast_err
mask = snr_less_5 <= 5
contrast_masked = ma.masked_array(contrast, mask)
contrast_err_masked = ma.masked_array(contrast_err, mask)

fig, axs = plt.subplots(nrows=2, ncols=1, figsize=(8, 5), sharex=False)
#fig, ax = plt.subplots()
#ax.plot(wl, contrast, '+', label='MATISSE contrast')
axs[0].errorbar(wl, contrast, yerr=contrast_err, color='lightgray', label='MATISSE contrast')
axs[0].errorbar(wl, contrast_masked, yerr=contrast_err_masked, color='crimson', label='S/N>5')
#ax.plot(wl, contrast, '+', c='tab:red')
#ax.set_ylim(0, 0.001)
axs[0].set_ylabel(r'Planet-to-star contrast')
#ax.set_ylim(1e-4, 12e-4)
#ax.set_ylim(3e-4, 12e-4)
axs[0].set_ylim(-0.8e-5, 2.1e-5)
#ax.set_xlim(2.9, 4.15)
axs[0].set_xlim(3., 4.1)
axs[0].set_xlabel(r'Wavelength [$\mu$m]')
axs[0].legend()

spec = np.loadtxt('/data/home/jscigliuto/Pipeline/Templates/HD72946/BT-NextGen_T5638K_lg4.51_M0.0_R9.09_res300.800.txt')
wl_star, spec_star = spec[:, 0], spec[:, 1]
interp_func = interp1d(wl_star, spec_star, kind='linear', bounds_error=False, fill_value="extrapolate")
spec_star_interp = interp_func(wl)

axs[1].plot(wl, spec_star_interp * 1e12, color='r', label='Stellar spectrum')
axs[1].set_ylabel(r'Flux [$\times$10$^{12}$ W/m$^2$/µm]')
axs[1].set_xlabel(r'Wavelength [$\mu$m]')
axs[1].legend()
fig.tight_layout()
fig.savefig(path_output + '/spectrometry/contrast_nochi2_masked.png')
...
# # Plot the GRAVITY and MATISSE planet spectrum
# # Load the GRAVITY spectrum

# with fits.open('/data/home/jscigliuto/Pipeline/Templates/HR8799_e_2024-09-29_GRAVITY.fits') as hdul:
#     data = hdul[1].data
#     wavelength_gravity = data['WAVELENGTH']
#     flux_gravity = data['FLUX']
#     flux_std_gravity = data['FLUX_STD']
# wl_all = wavelength_gravity + wl

# plt.figure(figsize=(15,5))
# plt.errorbar(wl_all, spec_planet_SI, yerr=spec_planet_err_SI, label='MATISSE spectrum')
# plt.errorbar(wl_all, flux_gravity, yerr=flux_std_gravity, fmt='o', label='Flux with error')
# plt.xlabel(r'Wavelength [$\mu$m]')
# plt.ylabel(r'Flux [W/m$^2$/µm]')
# plt.legend(loc='upper left')
# plt.ylim(-1e-15, 10e-15)
# plt.xlim(2.76, 5.0)
# plt.tight_layout()
# plt.savefig(path_output + '/spectrometry/spectrum_planet_simple_nochi2_SI.png')


# Plot the planet spectrum with S/N>5
# Remove data with S/R<5
spec_planet_masked = np.zeros_like(wl)
spec_planet_err_masked = np.zeros_like(wl)
maskL = (wl>=2.9) & (wl<=4.1) #& (wl>=4.47) & (wl<=5)
for iw in range(wl.size):
    snr = spec_planet[iw]/spec_planet_err[iw]
    if snr >= 5: 
        spec_planet_masked[iw] = spec_planet[iw]
        spec_planet_err_masked[iw] = spec_planet_err[iw]

spec_planet_masked_SI = spec_planet_masked * 1e-26 * (cst.c.value / (wl*1e-6)**2) * 1e-6
spec_planet_err_masked_SI = spec_planet_err_masked * 1e-26 * (cst.c.value / (wl*1e-6)**2) * 1e-6 

plt.figure(figsize=(15 , 5))
plt.errorbar(wl, spec_planet_SI, yerr=spec_planet_err_SI, label='MATISSE spectrum')
plt.errorbar(wl[maskL], spec_planet_masked_SI[maskL], yerr=spec_planet_err_masked_SI[maskL], color='r', label='MATISSE spectrum (S/N>5)')
plt.xlabel(r'Wavelength [$\mu$m]')
plt.ylabel(r'Flux [W/m$^2$/µm]')
plt.legend(loc='upper left')
# betaPicb
...
plt.xlim(2.76, 5.0)
plt.tight_layout()
plt.savefig(path_output + '/spectrometry/spectrum_planet_simple_nochi2_masked_SI.png')

# Planetary spectrum model (BT-Settl / 1700 K / 4.0)
planet_tpl = fits.getdata('/data/home/jscigliuto/Pipeline/Templates/HD72946/planet_spectrum_template_bt-settl_hd72946b.fits')
if use_bin_data:
...
# Save the spectra & covariance matrix
fits.writeto(path_output + '/spectrometry/contrast_planet_nochi2.fits', np.array([wl, contrast, contrast_err]), overwrite=True)
fits.writeto(path_output + '/spectrometry/spectrum_planet_nochi2.fits', np.array([wl, spec_planet_SI, spec_planet_err_SI]), overwrite=True)


# Save spectra & covariance for species
mask_wl = ((wl>2.9) & (wl<4.17)) | ((wl>4.5) & (wl<5.0))

wl_masked = wl[mask_wl]
contrast_cov = contrast_cov[mask_wl,:]
contrast_cov = contrast_cov[:,mask_wl]
spec_star_conv_SI = spec_star_conv_SI[mask_wl]
# contrast = contrast[mask_wl]
# contrast_err = contrast_err[mask_wl]
spec_planet_SI = spec_planet_SI[mask_wl]
spec_planet_err_SI = spec_planet_err_SI[mask_wl]

# contrast = contrast[::-1]
# contrast_err = contrast_err[::-1]
contrast_cov = contrast_cov[::-1, ::-1]
wl_masked = wl_masked[::-1]
spec_planet_SI = spec_planet_SI[::-1]
spec_planet_err_SI = spec_planet_err_SI[::-1]


# spec_planet_species = contrast * spec_star_conv_SI
# spec_planet_err_species = contrast_err * spec_star_conv_SI
spec_planet_cov_species = contrast_cov * np.outer(spec_star_conv_SI, spec_star_conv_SI)

fits.writeto(path_output + '/spectrometry/spectrum_planet_nochi2_species.fits', np.array([wl_masked, spec_planet_SI, spec_planet_err_SI]), overwrite=True)
fits.writeto(path_output + '/spectrometry/convariance_matrix.fits', np.array(spec_planet_cov_species), overwrite=True)
...
t.write(path_output + '/spectrometry/spectrum_MATISSE-LM.fits', format='fits', overwrite=True)
...
t.write(path_output + '/spectrometry/contrast_MATISSE-LM.fits', format='fits', overwrite=True)
...
# Add GRAVITY spectrum
with fits.open('/data/home/jscigliuto/Pipeline/Templates/HR8799_e_2024-09-29_GRAVITY.fits') as hdul:
    data = hdul[1].data
    wl_grav = data['WAVELENGTH']
    flux_grav = data['FLUX']
    flux_std_grav = data['FLUX_STD']


spec_ales = np.loadtxt("/data/home/jscigliuto/Pipeline/Templates/table_HR-8799-e-Doelman-et-al.-2022.csv", delimiter=",", skiprows=1, usecols=(0,2,3,4))
wl_ales, flux_ales, flux_err_sup, flux_err_inf = spec_ales[:,0], spec_ales[:,1], spec_ales[:,2], spec_ales[:,3]
flux_err_inf *= -1

spec_planet_SI /= 2.1/0.3
spec_planet_err_SI /= 2.1/0.3

wl_gravmat = np.hstack((wl_grav, wl[::-1]))
spec_gravmat = np.hstack((flux_grav, spec_planet_SI[::-1]))
spec_err_gravmat = np.hstack((flux_std_grav, spec_planet_err_SI[::-1]))

maskgravmat = ((wl_gravmat>1.9) & (wl_gravmat<2.5)) | ((wl_gravmat>3.35) & (wl_gravmat<4)) 


fig, (ax1, ax2) = plt.subplots(1, 2, sharey=True, figsize=(15, 5))
fig.subplots_adjust(wspace=0)
ax1.errorbar(wl_gravmat[maskgravmat], spec_gravmat[maskgravmat], yerr=spec_err_gravmat[maskgravmat], fmt='+', label='GRAVITY+MATISSE spectrum')
ax2.errorbar(wl_gravmat[maskgravmat], spec_gravmat[maskgravmat], yerr=spec_err_gravmat[maskgravmat], fmt='+', label='GRAVITY+MATISSE spectrum')
# ax2.errorbar(wl_ales, flux_ales, yerr=[flux_err_inf,flux_err_sup], fmt='+', label='Ales spectrum')
# plt.errorbar(wl_gravmat, spec_gravmat, yerr=spec_err_gravmat, fmt='--', label='GRAVITY+MATISSE spectrum')

plt.legend(loc='upper right')
plt.ylim(-0.3e-16, 2.2e-16)
ax1.set_xlim(1.8, 2.6)
ax1.axvline(x=2.6, linewidth=4, color='red')
ax2.set_xlim(3.3, 4.1)
ax2.axvline(x=3.3, linewidth=4, color='red')
ax1.spines['right'].set_visible(False)
ax2.spines['left'].set_visible(False)
ax2.yaxis.set_visible(False) 
fig.supxlabel(r'Wavelength [$\mu$m]')
fig.supylabel(r'Flux [W/m$^2$/µm]')

plt.tight_layout()
plt.savefig(path_output + '/spectrometry/spectrum_planet_GRAVITY+MATISSE.png')
