...
path_oifits = '/Users/jscigliuto/Nextcloud/DATA/HD72946B/corrected_data_5bin/' #HD 72946 B
# path_oifits = '/Users/jscigliuto/Nextcloud/DATA/betaPicb/corrPhase_OB1/' #beta Pic b
...
for directory in [ '/Amplitude', '/Phase', '/Real_part', '/Imag_part', '/Err_comparison']:
...
Cps_real_err_quad_all = np.zeros((n_file_planet, 6, n_wave))
Cps_imag_err_quad_all = np.zeros((n_file_planet, 6, n_wave))
...
if target == 'betaPicb':
    ...
    for n_file, file_planet in enumerate(files_planet):
        ...
        dRe_star = (cf_planet_real * cf_star_interp_imag**2 - cf_planet_real * cf_star_interp_real**2 - 2 * cf_star_interp_real * cf_star_interp_imag * cf_planet_imag) / (norm**2)
        # dIm_star = (cf_planet_real * cf_star_interp_imag - cf_planet_imag * cf_star_interp_real) * 2 * cf_star_interp_imag / (norm**2)
        dIm_star = (cf_planet_imag * cf_star_interp_real**2 - cf_planet_imag * cf_star_interp_imag**2 - 2 * cf_star_interp_real * cf_star_interp_imag * cf_planet_real) / (norm**2)
        ...
        Cps_real_err_quad = np.sqrt((cf_real_planet_err / np.real(cf_planet))**2 + (cf_real_star_err / np.real(cf_star))**2) * np.abs(np.real(Cps))
        ...
        dRe_star = (cf_planet_imag * cf_star_interp_imag**2 - cf_planet_imag * cf_star_interp_real**2 + 2 * cf_star_interp_real * cf_star_interp_imag * cf_planet_real) / (norm**2)
        # dIm_star = -(cf_planet_real * cf_star_interp_imag + cf_planet_imag * cf_star_interp_real) * 2 * cf_star_interp_imag / (norm**2)
        dIm_star = (cf_planet_real * cf_star_interp_real**2 - cf_planet_real * cf_star_interp_imag**2 + 2 * cf_star_interp_real * cf_star_interp_imag * cf_planet_imag) / (norm**2)
        ...
        Cps_imag_err_quad = np.sqrt((cf_imag_planet_err / np.imag(cf_planet))**2 + (cf_imag_star_err / np.imag(cf_star))**2) * np.abs(np.imag(Cps))
        ...
        Cps_real_err_quad_all[n_file] = Cps_real_err_quad
        Cps_imag_err_quad_all[n_file] = Cps_imag_err_quad
        ...
        if plot == True:
            ...
            fig_err, ax_err = plt.subplots(6, 1, figsize=(12, 8), sharex=True)
            ax_err = ax_err.flatten()

            fig_err_resid, ax_err_residue = plt.subplots(6, 1, figsize=(12, 8), sharex=True)
            ax_err_resid = ax_err_residue.flatten()
            ...
            for i_base in range(6): 
                ...
                #Compoarison of error estimation methods
                fig_err.suptitle('Comparison of Error Estimation Methods', fontsize=16)
                if i_base < 5:   
                    ax_err[i_base].tick_params(labelbottom=False)
                if i_base == 5:
                    ax_err[i_base].set_xlabel('Wavelength (µm)', fontsize=12)
                    ax_err[i_base].plot(wl*1e6, Cps_real_err_all[n_file, i_base], label='Full error propagation', color='teal', alpha=0.8)
                    ax_err[i_base].plot(wl*1e6, Cps_real_err_quad_all[n_file, i_base], label='Quadratic sum approximation', color='orange', alpha=0.8)
                ax_err[i_base].plot(wl*1e6, Cps_real_err_all[n_file, i_base], color='teal', alpha=0.8)
                ax_err[i_base].plot(wl*1e6, Cps_real_err_quad_all[n_file, i_base], color='orange', alpha=0.8)
                ax_err[i_base].set_ylim(0, 4e-3)
                ax_err[i_base].set_xlim(2.9, 5.0)
                ax_err[i_base].set_title(f'Baseline {base_order_name[i_base]}')
                ax_err[i_base].legend(loc='upper right')

                fig_err_resid.suptitle('Residuals between Error Estimation Methods', fontsize=16)
                if i_base < 5:   
                    ax_err_resid[i_base].tick_params(labelbottom=False)
                if i_base == 5:
                    ax_err_resid[i_base].set_xlabel('Wavelength (µm)', fontsize=12)
                ax_err_resid[i_base].plot(wl*1e6, Cps_real_err_all[n_file, i_base] - Cps_real_err_quad_all[n_file, i_base], color='brown', alpha=0.8)
                ax_err_resid[i_base].set_title(f'Baseline {base_order_name[i_base]}')
                ax_err_resid[i_base].set_xlim(2.9, 5.0)
                ax_err_resid[i_base].set_ylim(-1e-3, 1e-3)
                fig_err_resid.suptitle('Residuals between Error Estimation Methods', fontsize=16)
            ...
            fig_err.savefig(path_output + f'/Plots/Err_comparison/Cps_err_comparison_OB{num_OB_planet}_exp{num_exp_planet}_frame{num_frame_planet}.png', dpi=300)
            fig_err_resid.savefig(path_output + f'/Plots/Err_comparison/Cps_err_resid_OB{num_OB_planet}_exp{num_exp_planet}_frame{num_frame_planet}.png', dpi=300)
...
######################################################################################################################################################################################################
    ...
    for n_file, file_planet in enumerate(files_planet):
        ...
        Cps_real_err_quad = np.sqrt((cf_real_planet_err / np.real(cf_planet))**2 + (cf_real_star_err / np.real(cf_star_interp))**2) * np.abs(np.real(Cps))
        ...
        Cps_imag_err_quad = np.sqrt((cf_imag_planet_err / np.imag(cf_planet))**2 + (cf_imag_star_err / np.imag(cf_star_interp))**2) * np.abs(np.imag(Cps))
        ...
        if plot == True:
            ...
            for i_base in range(6): 
                ...
                ax_real[i_base].fill_between(wl*1e6, np.real(Cps_all[n_file,i_base])-Cps_real_err_all[n_file, i_base], 
                                        np.real(Cps_all[n_file,i_base])+Cps_real_err_all[n_file, i_base], alpha=0.2, color='lightblue')
                ax_real[i_base].set_ylim(-1e-2, 6e-2)
                ax_real[i_base].set_title(f'Baseline {base_order_name[i_base]}')
                ...
                ax_imag[i_base].fill_between(wl*1e6, np.imag(Cps_all[n_file,i_base])-Cps_imag_err_all[n_file, i_base], 
                                        np.imag(Cps_all[n_file,i_base])+Cps_imag_err_all[n_file, i_base], alpha=0.2, color='violet')
                ax_imag[i_base].set_ylim(-4e-2, 4e-2)
                ax_imag[i_base].set_title(f'Baseline {base_order_name[i_base]}')
                ...
                if i_base == 5:
                    ...
                    ax_err[i_base].plot(wl*1e6, Cps_real_err_all[n_file, i_base], label='Full error propagation', color='teal', alpha=0.8)
                    ax_err[i_base].plot(wl*1e6, Cps_real_err_quad_all[n_file, i_base], label='Quadratic sum approximation', color='orange', alpha=0.8)
                ax_err[i_base].plot(wl*1e6, Cps_real_err_all[n_file, i_base], alpha=0.8)
                ax_err[i_base].plot(wl*1e6, Cps_real_err_quad_all[n_file, i_base], color='orange', alpha=0.8)
                ax_err[i_base].set_ylim(0, 4e-2)
                ax_err[i_base].set_xlim(2.9, 5.0)
                ax_err[i_base].set_title(f'Baseline {base_order_name[i_base]}')
                ax_err[i_base].legend(loc='upper right')
