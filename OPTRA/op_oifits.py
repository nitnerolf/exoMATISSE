...
##############################################
# 
def op_gen_oiwavelength(cfdata, wlen_fin='EFF_WAVE' ,verbose=True, plot=False): #wlen_fin='EFF_WAVE_Binned'
...
def op_gen_oivis(cfdata, cfin='CF_achr_phase_corr', verbose=True, plot=False):
    ...
    oivis_table['UCOORD']    = np.array(cfdata['OI_BASELINES']['UCOORD']).reshape(nframes*nbases)
    oivis_table['VCOORD']    = np.array(cfdata['OI_BASELINES']['VCOORD']).reshape(nframes*nbases)
